...
def run_vgg(i):
    ...
    q0_1 = "select (((avg(val))/stddevSamp(val))+0.00005)>0?1:0 \
        from ( \
            select sum(cv1 * k1)>0?1:0 as val, bn1 \
            from dl2sql_model"+str(i)+"_conv1_2 \
            group by bn1 \
        ) \
        group by bn1;"
    q0_2 = " select (((avg(val_cv2))/stddevSamp(val_cv2))+0.00005)>0?1:0 \
        from ( \
            select any(im_id) as im_id2, kid2, sum(cv2*kv2)>0?1:0 as val_cv2 \
            from dl2sql_model"+str(i)+"_conv2_data2_2 as a inner join dl2sql_model"+str(i)+"_conv2_kernel2_2 as b \
            on a.bin_id = b.orderid2 \
            group by cn_id, kid2 \
        ) \
        group by im_id2, kid2;"
    q0_3 = " select (((avg(val_cv2))/stddevSamp(val_cv2))+0.00005)>0?1:0 \
        from ( \
            select any(im_id) as im_id2, kid2, sum(cv2*kv2)>0?1:0 as val_cv2 \
            from dl2sql_model"+str(i)+"_conv3_data2_2 as a inner join dl2sql_model"+str(i)+"_conv3_kernel2_2 as b \
            on a.bin_id = b.orderid2 \
            group by cn_id, kid2 \
        ) \
        group by im_id2, kid2;"
    q0_4 = "select (((avg(val_pool))/stddevSamp(val_pool))+0.00005)>0?1:0 \
        from ( \
            ...
            from ( \
                select any(im_id) as im_id2, any(pool2) as pool22, cn_id, kid2, sum(cv2*kv2)>0?1:0 as val_cv2 \
                from dl2sql_model"+str(i)+"_conv4_data2_2 as a inner join dl2sql_model"+str(i)+"_conv4_kernel2_2 as b \
                on a.bin_id = b.orderid2 \
                group by cn_id, kid2 \
            ) \
            group by cn_id, pool22) \
    ...
    q0_5 = " select (((avg(val_cv2))/stddevSamp(val_cv2))+0.00005)>0?1:0 \
        from ( \
            select any(im_id) as im_id2, kid2, sum(cv2*kv2)>0?1:0 as val_cv2 \
            from dl2sql_model"+str(i)+"_conv5_data2_2 as a inner join dl2sql_model"+str(i)+"_conv5_kernel2_2 as b \
            on a.bin_id = b.orderid2 \
            group by cn_id, kid2 \
        ) \
        group by im_id2, kid2;"
    q0_6 = "select (((avg(val_pool))/stddevSamp(val_pool))+0.00005)>0?1:0 \
        from ( \
            ...
            from ( \
                select any(im_id) as im_id2, any(pool2) as pool22, cn_id, kid2, sum(cv2*kv2)>0?1:0 as val_cv2 \
                from dl2sql_model"+str(i)+"_conv6_data2_2 as a inner join dl2sql_model"+str(i)+"_conv6_kernel2_2 as b \
                on a.bin_id = b.orderid2 \
                group by cn_id, kid2 \
            ) \
            group by cn_id, pool22) \
    ...
    q0_7 = " select (((avg(val_cv2))/stddevSamp(val_cv2))+0.00005)>0?1:0 \
        from ( \
            select any(im_id) as im_id2, kid2, sum(cv2*kv2)>0?1:0 as val_cv2 \
            from dl2sql_model"+str(i)+"_conv7_data2_2 as a inner join dl2sql_model"+str(i)+"_conv7_kernel2_2 as b \
            on a.bin_id = b.orderid2 \
            group by cn_id, kid2 \
        ) \
        group by im_id2, kid2;"
    q0_8 = " select (((avg(val_cv2))/stddevSamp(val_cv2))+0.00005)>0?1:0 \
        from ( \
            select any(im_id) as im_id2, kid2, sum(cv2*kv2)>0?1:0 as val_cv2 \
            from dl2sql_model"+str(i)+"_conv8_data2_2 as a inner join dl2sql_model"+str(i)+"_conv8_kernel2_2 as b \
            on a.bin_id = b.orderid2 \
            group by cn_id, kid2 \
        ) \
        group by im_id2, kid2;"
    q0_9 = "select (((avg(val_cv2))/stddevSamp(val_cv2))+0.00005)>0?1:0 \
        from ( \
            select any(im_id) as im_id2, kid2, sum(cv2*kv2)>0?1:0 as val_cv2 \
            from dl2sql_model"+str(i)+"_conv9_data2_2 as a inner join dl2sql_model"+str(i)+"_conv9_kernel2_2 as b \
            on a.bin_id = b.orderid2 \
            group by cn_id, kid2 \
        ) \
        group by im_id2, kid2;"
    q0_10 = "select (((avg(val_cv2))/stddevSamp(val_cv2))+0.00005)>0?1:0 \
        from ( \
            select any(im_id) as im_id2, kid2, sum(cv2*kv2)>0?1:0 as val_cv2 \
            from dl2sql_model"+str(i)+"_conv10_data2_2 as a inner join dl2sql_model"+str(i)+"_conv10_kernel2_2 as b \
            on a.bin_id = b.orderid2 \
            group by cn_id, kid2 \
        ) \
        group by im_id2, kid2;"
    q0_11 = " select (((avg(val_cv2))/stddevSamp(val_cv2))+0.00005)>0?1:0 \
        from ( \
            select any(im_id) as im_id2, kid2, sum(cv2*kv2)>0?1:0 as val_cv2 \
            from dl2sql_model"+str(i)+"_conv11_data2_2 as a inner join dl2sql_model"+str(i)+"_conv11_kernel2_2 as b \
            on a.bin_id = b.orderid2 \
            group by cn_id, kid2 \
        ) \
        group by im_id2, kid2;"
    q0_12 = " select (((avg(val_cv2))/stddevSamp(val_cv2))+0.00005)>0?1:0 \
        from ( \
            select any(im_id) as im_id2, kid2, sum(cv2*kv2)>0?1:0 as val_cv2 \
            from dl2sql_model"+str(i)+"_conv12_data2_2 as a inner join dl2sql_model"+str(i)+"_conv12_kernel2_2 as b \
            on a.bin_id = b.orderid2 \
            group by cn_id, kid2 \
        ) \
        group by im_id2, kid2;"
    q0_13 = "select (((avg(val_cv2))/stddevSamp(val_cv2))+0.00005)>0?1:0 \
        from ( \
            select any(im_id) as im_id2, kid2, sum(cv2*kv2)>0?1:0 as val_cv2 \
            from dl2sql_model"+str(i)+"_conv13_data2_2 as a inner join dl2sql_model"+str(i)+"_conv13_kernel2_2 as b \
            on a.bin_id = b.orderid2 \
            group by cn_id, kid2 \
        ) \
        group by im_id2, kid2;"
...
if __name__ == '__main__':
    ...
    run_vgg(id)
    t2 = time.time()
    print("Model Number: {}, Memory Usage: {} MB, Time: {}".format(1, get_current_memory_usage(), t2-t1))
    print("-----------------end-----------------")
