...
for i in range (1,129):
    str_linear = str_linear + "lookup(data2["+str(i)+"], kv2["+str(i)+"])"
    if i!=128:
...
def run_vgg_global(i):
    ...
    q0_14 = str_linear + "from (select data2, kv1 as kv2 \
        ...
        from ( \
            select any(ln_id1) as ln_id2, groupBitAnd(lv1) as data1 from model"+str(i)+"_linear1_ln1_2 group by bin_id) \
        group by ln_id2) as t_l2, model"+str(i)+"_linear1_kerne_ln1_2);"
    q0_15 = "select (lookup(data2[1], kv2[1])+lookup(data2[2], kv2[2]) \
        +lookup(data2[3], kv2[3])+lookup(data2[4], kv2[4])+lookup(data2[5], kv2[5]) \
        +lookup(data2[6], kv2[6])+lookup(data2[7], kv2[7])+lookup(data2[8], kv2[8]) \
        +lookup(data2[9], kv2[9])+lookup(data2[10], kv2[10])+lookup(data2[11], kv2[11]) \
        +lookup(data2[12], kv2[12])+lookup(data2[13], kv2[13])+lookup(data2[14], kv2[14]) \
        ...
        from ( \
            select any(ln_id1) as ln_id2, groupBitAnd(lv1) as data1 from model"+str(i)+"_linear2_ln1_2 group by bin_id) \
        group by ln_id2) as t_l2, model"+str(i)+"_linear2_kerne_ln1_2);"
    q0_16 = "select (lookup(data2[1], kv2[1])+lookup(data2[2], kv2[2]) \
        +lookup(data2[3], kv2[3])+lookup(data2[4], kv2[4])+lookup(data2[5], kv2[5]) \
        +lookup(data2[6], kv2[6])+lookup(data2[7], kv2[7])+lookup(data2[8], kv2[8]) \
        +lookup(data2[9], kv2[9])+lookup(data2[10], kv2[10])+lookup(data2[11], kv2[11]) \
        +lookup(data2[12], kv2[12])+lookup(data2[13], kv2[13])+lookup(data2[14], kv2[14]) \
